...
void telemetry() {
    ...
    RoveComm.write(RC_PMSBOARD_PACKCURRENT_DATA_ID, -packCurrent); // someone put the current sensor in backwards
...
void readCells() {
    ...
    // Calculate pack voltage; global variable is used in interrupt, so it should only be changed after the total is
    // calculated
    float pv = 0;
    for (uint8_t i = 0; i < NUM_CELLS; i++) {
        pv += cellVoltages[i];
